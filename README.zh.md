...
3. 展开右侧边栏，「开始」页上就有 **「后台任务统计」** 卡片。宿主会自行重新读取变更过的客户端 bundle，**不需要重启**。

包内的 `cordis.patch.yml` 只做一件事：向组合插入一条 Loader 行（`job-stats` → `dsh-client-ui-job-stats`），不改动 profile 里的其它任何东西。
...
- 台账只记录**标签页开着**时看到过的任务；某个任务若在标签页关闭期间开始并结束，名册不会补发，面板也就无从统计。
- **前台命令的结局无法观测**：工具在调用方取走输出时就把记录从名册删掉了，而名册流是合并后再读列表的，所以"已结算"这一帧读不到 —— 面板只看到它"运行中"，随后就不见了。这类记录显示为**已结束（结果未上报）**，既不算运行中，也不会被硬猜成已完成或已失败；因此「完成率」只覆盖已上报结果的结算任务。保留记录的任务（`run_in_background`、超时转后台、尚未被取走输出的任务）会给出真实终态。
- 「已结束」记录的耗时 = 从开始到最后一次被看到（也就是宿主把它撤下名册的时刻，误差约一秒内）。
- 每会话上限 300 条，超出先淘汰最早已结束的记录。
...
| `cordis.patch.yml` | 插入一条 Loader 行：`job-stats` → `dsh-client-ui-job-stats` |
| `lib/index.js` | 宿主半侧：Loaders 可见的空实现；功能全在浏览器侧 |
