...
| | |
|---|---|
| 计数 | 合计 / 运行中 / 已完成 / 已失败 / 已取消 |
| 指标 | 完成率（已完成 ÷ 已结算）、累计耗时、最长耗时、保留输出字节数 |
...
3. 统计与明细都从台账算出，所以任务被宿主移除后仍留在面板里。台账持久化在浏览器存储的 `dsh-job-stats/v1/<sessionId>` 下（写入有节流；存储写满或损坏时自动退回"仅内存"）。

### 已知限制

- 台账只记录**标签页开着**时看到过的任务；某个任务若在标签页关闭期间开始并结束，名册不会补发，面板也就无从统计。
- 仍标记为"运行中"但已离开名册的记录，耗时冻结在最后一次看到的时间点（不会无限增长），状态保留最后一次看到的值。
