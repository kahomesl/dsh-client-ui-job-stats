...
- **The ledger now finds its own file, and a burst of settlements always reaches it.**
  ...
  - **Job identity survives a Host restart.** Registry ids such as `pwsh-1` restart at 1 whenever its process-wide registry instance is recreated. The recorder stamps a full random UUID per boot and keys each outcome by `bootId:owner:id` (with a distinct unowned marker); the raw `id` stays visible in task details. Host ledger schema is `dsh-job-stats/ledger/v2`, and surviving v1 records are converted rather than discarded. The browser stores `dsh-job-stats/v3/<sessionId>`, migrates surviving v2 records to stable legacy identities without deleting v2, uses a provisional `sessionId:id:startedAt` identity for live roster rows, and upgrades it to the Host canonical key when the matching settlement arrives. Two channels cannot duplicate the same job or let a stale frame downgrade a Host outcome. Previously overwritten historical records cannot be reconstructed.
  - A local-only, one-time staged import can merge a verified pre-restart Host outcomes backup into the running recorder and synchronously flush it. It never exposes an HTTP write endpoint: an external operator script first backs up the v2 ledger, refuses an impossible 200/32-capacity import or an oversized stage, and stages a validated file in the profile; the recorder ingests it on its next route read, removes the stage only after a successful write, and replays safely after a crash.
